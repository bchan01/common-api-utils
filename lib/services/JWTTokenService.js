...
JWTTokenService.prototype.validateRequest = function(req, res, next) {
    // needed req.baseUrl, saved res.locals.tokenPayload
        var jwtoken = req.header('x-access-token');
        //var secrets = [];

        res.locals.tokenPayload = jwt.decode(jwtoken);
        res.locals.tokenValid = false;
      
        //secrets.push(this.secret);
        console.log('validateRequest - Secret:++' + this.secret + '+++JWT:' + jwtoken);

        var valid = validate(jwtoken, this.secret);
